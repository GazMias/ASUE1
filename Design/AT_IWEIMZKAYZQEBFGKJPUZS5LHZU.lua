--Объявление структуры, в	 которой левая часть до знака = это массив сигналов из ПЛК S7-300 а правая часть - это массив переменных, передаваемых в SCADA систему
local objects = {
	...
	["VV1_OTKVV_CO"] = 
		...
		function(Name)
		  	 if  Core[Name[2].."VV1_OTKVV_CO"]==true then		-- Проверка на то, послали ли команду
			     Core[Name[1].."COMMAND1_CO"]=1					-- Записываем слово управления (значение слова=2 в степени N, где N -номер бита команды в слове)
			 	 os.sleep(0.5) 									-- Таймаут после посылки команды 0.5 секунды
			 	 Core[Name[1].."COMMAND1_CO"]=0					-- Обнуляем слово управления во избежание ложных срабатываний
	...
	["VV1_VKLVV_CO"] = 
		...
		function(Name)
			 if  Core[Name[2].."VV1_VKLVV_CO"]==true then
			     Core[Name[1].."COMMAND1_CO"]=2
			 	 os.sleep(0.5)
			 	 Core[Name[1].."COMMAND1_CO"]=0
...
-- Управление АВР АВ2
	["AV2_OTKAVRAV_CO"] = 
					   {["comment"]="Отключить АВР АВ 2",["eval"]=
		function(Name)
			 if  Core[Name[2].."AV2_OTKAVRAV_CO"]==true then
			     Core[Name[1].."COMMAND2_CO"]=2
			 	 os.sleep(0.5)
			 	 Core[Name[1].."COMMAND2_CO"]=0
